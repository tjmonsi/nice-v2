...
class CbeRecommendsPage extends Permission(User(Polymer.Element))  {
  ...
  addItem () {
    ...
    return firebase.database().ref().update(updates).then(() => {
        document.querySelector('app-shell').showMessage('Created a CBE Recommends Article', null, null, null, 5000)
        window.history.pushState({}, '', `cberecommends/${key}/edit`)
